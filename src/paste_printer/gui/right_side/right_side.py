...
class Right_Side(QWidget):
    ...
    def initUI(self):
        ...
        self.turn_on_off_button = QPushButton("Turn Animation Off")
    ...
    def turn_on_off_animation(self, text):
        ...
        if text == "Turn Animation On":
            ...
            self.animation = True
        self.gcode_layer_viewer.load_layer(self.current_layer, self.animation)
    ...
    def load_layer(self, index):
        if index == 1 and self.current_layer == self.max_layer:
            self.current_layer = 0
