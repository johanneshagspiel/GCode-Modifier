...
class MainScreen_TKinter:
    ...
    def __init__(self):
        ...
        self.result_retract_syringe_end_of_print = IntVar()
        self.retract_syringe_end_of_print_checkbox = Checkbutton(self.print_modifications_frame, text="Retract the syringe at the end of print",
                                                                 variable=self.result_retract_syringe_end_of_print, **Styles.checkbox_style)
        self.retract_syringe_end_of_print_checkbox.grid(row=row_position_index_print_modifications_frame, column=0, sticky='w')
        row_position_index_print_modifications_frame += 1
    ...
    def sanity_check(self):
        ...
        if len(messages) > 0:
            ...
            file_name_with_extension = self.selected_file.get() + ".gcode"
            path_to_file = Path.joinpath(self.file_handler.gcode_path, file_name_with_extension)
            add_information = self.result_add_information.get()
            stop_each_layer = self.result_stop_after_each_layer.get()
            retract_syringe_end_of_print = self.result_retract_syringe_end_of_print.get()

            return Command(path_to_file=path_to_file,
                           flow_rate=int_flow_rate, bed_temperature=int_bed_temperature,
                           additional_information_bol=add_information,
                           pause_each_layer_bol=stop_each_layer,
                           retract_syringe_bol=retract_syringe_end_of_print,
